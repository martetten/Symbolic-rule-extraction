...
class TopKSAE(torch.nn.Module):
    """Top-K SAE с опциональным центрированием активаций"""
    def __init__(self, d_in, d_sae, k, mean=None):
        super().__init__()
        self.k = k
        if mean is not None:
            # Регистрируем как буфер, чтобы сохранялся в state_dict и перемещался с моделью
